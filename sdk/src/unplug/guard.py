...
class Guard:
        ...
        *,
        ...
        if cfg.mode != "server" and cfg.active_model:
            ...
            try:
                ...
                self._warn_ml_degraded(cfg.active_model, exc)
            if provider is not None and spec is not None:
                ...
                self._warn_ml_degraded(cfg.active_model)
    ...
    def _warn_ml_degraded(self, active_model: str, exc: Exception | None = None) -> None:
        """Log the active_model/ML-not-loaded warning once and mark ML degraded.

        ``exc`` is the load failure when one was caught; omitted when the
        model simply never got wired up (no failure to report).
        """
        detail = f"failed to load ({type(exc).__name__})" if exc is not None else "is not loaded"
        _log.warning(
